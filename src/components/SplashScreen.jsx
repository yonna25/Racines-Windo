    ...
    <div className="relative w-full h-screen overflow-hidden select-none"
      ...
      {STARS.map(s => (
        <div key={s.id} className="absolute rounded-full bg-white"
             style={{ top: s.top, left: s.left, width: s.size, height: s.size, opacity: 0.7,
                      animation: `twinkle ${s.duration} ${s.delay} ease-in-out infinite` }} />
      ))}
      <div className="absolute rounded-full"
           style={{ top: '8%', left: '72%', width: 52, height: 52,
      ...
      <div className="absolute rounded-full"
           style={{ top: '14%', left: '30%', width: 6, height: 6,
                    background: '#FFF5B0', boxShadow: '0 0 8px rgba(255,245,176,0.8)' }} />
      <div className="absolute bottom-0 left-0 right-0"
           style={{ height: '45%', background: 'linear-gradient(0deg,rgba(232,160,48,0.18) 0%,transparent 100%)', pointerEvents: 'none' }} />
      <div className="absolute" style={{ bottom: '22%', left: '50%', transform: 'translateX(-50%)', width: 260, opacity: 0.92 }}>
      ...
      <div className="absolute bottom-[21%] left-0 right-0" style={{ height: 90 }}>
        {FIGURES.map(f => (
          <div key={f.id} className="absolute bottom-0"
            ...
            <svg viewBox={`0 0 ${f.w} ${f.h}`} fill="none" className="w-full h-full">
              <path d={f.path} stroke="#0D0602" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill="#0D0602" fillOpacity="0.15"/>
            </svg>
          </div>
        ))}
      </div>
      <div className="absolute bottom-0 left-0 right-0"
        ...
        {[5,12,22,35,48,58,70,80,90].map((x, i) => (
          <div key={i} className="absolute bottom-full"
               style={{ left: `${x}%`, width: 1, height: 8+(i%3)*4, background: '#3A2A08',
                        borderRadius: 2, transform: `rotate(${(i%2===0?-1:1)*8}deg)` }} />
        ))}
      </div>
      <div className="absolute bottom-0 left-0 right-0 flex flex-col items-center pb-12 z-50"
           style={{ background: 'linear-gradient(0deg,rgba(26,14,4,0.95) 0%,rgba(26,14,4,0.7) 60%,transparent 100%)', paddingTop: 48 }}>
        <p className="font-display text-5xl font-light tracking-[0.3em] uppercase mb-1"
           style={{ color: '#E2C99A', animation: 'fadeUp 1s ease 0.5s both' }}>Racines</p>
        <div className="flex items-center gap-3 mb-3" style={{ animation: 'fadeUp 1s ease 0.7s both' }}>
        ...
        <p className="font-display italic text-sm font-light tracking-widest mb-8"
           style={{ color: 'rgba(200,169,110,0.55)', animation: 'fadeUp 1s ease 0.9s both' }}>Retrace tes origines</p>
        {phase === 'ready' ? (
          <button onClick={onComplete}
                  ...
                  style={{ color: '#C8A96E', borderColor: 'rgba(200,169,110,0.4)', background: 'rgba(200,169,110,0.06)',
                           boxShadow: '0 0 24px rgba(200,169,110,0.1)', animation: 'fadeUp 0.7s ease both' }}>
            Commencer
          </button>
        ) : (
        ...
        <p className="font-sans text-xs mt-6"
           style={{ color: 'rgba(200,169,110,0.2)', letterSpacing: '0.15em', animation: 'fadeUp 1s ease 1.3s both' }}>v1.0.0</p>
      </div>
    </div>
  )
                   }
